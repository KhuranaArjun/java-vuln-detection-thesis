#!/usr/bin/env python3
"""
Standalone Enhanced CNN Java Vulnerability Detection Trainer
Incorporates all BiLSTM improvements: AST features, 256-token sequences, focal loss,
framework-aware classification, and Java-specific optimizations
"""
...
from tensorflow.keras.layers import (
    ...
    Concatenate, BatchNormalization, SpatialDropout1D, GlobalAveragePooling1D
)
...
class EnhancedCNNVulnerabilityClassifier:
    ...
    def train_word2vec_embeddings(self, samples: List[Dict]) -> Word2Vec:
        ...
        word2vec_path = self.model_output_dir / "cnn_java_word2vec.model"
    ...
    def create_vocabulary(self, samples: List[Dict]) -> Dict[str, int]:
        ...
        vocab_path = self.model_output_dir / "cnn_vocabulary.pkl"
    ...
    def prepare_dual_input_sequences(self, samples: List[Dict]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Prepare sequences for dual-input CNN model"""
    ...
    def build_enhanced_cnn_model(self, embedding_matrix: np.ndarray) -> tf.keras.Model:
        """Build enhanced CNN model with multiple filter sizes"""
        ...
        token_embedding = SpatialDropout1D(0.2)(token_embedding)
        ...
        semantic_embedding = SpatialDropout1D(0.2)(semantic_embedding)
        
        # Multiple CNN filter sizes for different vulnerability pattern scales
        filter_sizes = [2, 3, 4, 5, 8, 12]
        cnn_outputs = []
        ...
        for filter_size in filter_sizes:
            ...
            conv_tokens = Conv1D(
                ...
                activation='relu',
                padding='same',
                name=f'conv_tokens_{filter_size}'
            )(token_embedding)
            conv_tokens = BatchNormalization()(conv_tokens)
            pool_tokens = GlobalMaxPooling1D()(conv_tokens)
            
            # Semantic-based CNN
            conv_semantic = Conv1D(
                filters=64,
                kernel_size=filter_size,
                activation='relu',
                padding='same',
                name=f'conv_semantic_{filter_size}'
            )(semantic_embedding)
            conv_semantic = BatchNormalization()(conv_semantic)
            pool_semantic = GlobalMaxPooling1D()(conv_semantic)
            
            # Combine token and semantic features
            combined = Concatenate()([pool_tokens, pool_semantic])
            cnn_outputs.append(combined)
        
        # Merge all filter outputs
        merged = Concatenate()(cnn_outputs)
        merged = Dropout(self.dropout_rate)(merged)
        
        # Framework-aware dense layers
        framework_dense = Dense(128, activation='relu', name='framework_features')(merged)
        framework_dense = Dropout(self.dropout_rate)(framework_dense)
        
        # Main classification layers
        dense1 = Dense(256, activation='relu', name='dense_1')(framework_dense)
        ...
        # Output layer
        outputs = Dense(len(self.vulnerability_classes), activation='softmax', name='vulnerability_output')(dense2)
        ...
        logger.info("Enhanced CNN model architecture:")
        model.summary()
    ...
    def train_cnn_model(self) -> Dict:
        ...
        # Build CNN model
        model = self.build_enhanced_cnn_model(embedding_matrix)
        ...
        callbacks = [
            ...
            ModelCheckpoint(
                str(self.model_output_dir / 'best_cnn_model.keras'), 
                monitor='val_accuracy', 
                save_best_only=True, 
                mode='max',
                verbose=1
        ...
        logger.info("Evaluating CNN model on test set...")
        ...
        # Save results
        results_path = self.model_output_dir / "cnn_training_results.json"
        with open(results_path, 'w') as f:
            json.dump(results, f, indent=2, default=str)
        
        # Save model
        model.save(str(self.model_output_dir / "enhanced_cnn_vulnerability_classifier.keras"))
        
        logger.info(f"Enhanced CNN training complete! Results saved to {self.model_output_dir}")
        logger.info(f"CNN Test Accuracy: {test_accuracy:.4f}")
        logger.info(f"CNN F1-Score (Macro): {f1_macro:.4f}")
        logger.info(f"CNN F1-Score (Weighted): {f1_weighted:.4f}")
        
        print("\n" + "="*80)
        print("ENHANCED CNN MODEL CLASSIFICATION REPORT")
        print("="*80)
        print(class_report)
        
        print("\n" + "="*80)
        print("CNN MODEL ENHANCEMENT SUMMARY")
        print("="*80)
        print(f"Architecture: Multi-scale CNN with 6 filter sizes")
        print(f"Input Features: Token embeddings + AST semantic features")
        print(f"Sequence Length: {self.max_sequence_length} tokens")
        print(f"Embedding Dimension: {self.embedding_dim}")
        print(f"Filter Sizes: [2, 3, 4, 5, 8, 12] tokens")
...
def main():
    ...
    model_output_dir = "/Users/ARJUN/java-vulnerability-detection-backup/models/enhanced_cnn_java"
    
    # Initialize CNN trainer
    trainer = EnhancedCNNVulnerabilityClassifier(dataset_path, model_output_dir)
    ...
    try:
        ...
        print("\n" + "="*80)
        print("ENHANCED CNN JAVA VULNERABILITY CLASSIFICATION COMPLETE")
        print("="*80)
        print(f"CNN Test Accuracy: {results['test_metrics']['accuracy']:.4f}")
        print(f"CNN F1-Score (Macro): {results['test_metrics']['f1_score_macro']:.4f}")
        print(f"CNN F1-Score (Weighted): {results['test_metrics']['f1_score_weighted']:.4f}")
        print(f"CNN Precision (Weighted): {results['test_metrics']['precision_weighted']:.4f}")
        print(f"CNN Recall (Weighted): {results['test_metrics']['recall_weighted']:.4f}")
        print(f"\nCNN model saved to: {model_output_dir}")
        print(f"Number of vulnerability classes: {results['dataset_info']['num_classes']}")
        print(f"Architecture: Multi-scale CNN with dual-input (tokens + AST features)")
